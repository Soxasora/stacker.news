...
- **C3 ✅ done (2026-07-07; implementation landed with the spec in the "amend" commit, gates closed same day).** As landed, verbatim from §12.1–12.3: `ui/tooltip.js` + `tooltip.module.css` (COLLISIONS_OFF, `sideOffset={6}`, `text-sm max-w-48 rounded-md` recipe, diamond arrow, 150ms starting/ending-style motion + reduced-motion off-switch), `TooltipProvider delay={0} closeDelay={0}` wrapping `_app.js`, `action-tooltip.js` internals (API frozen; conditional `useFormikContext` kept verbatim; `transition`/`hideDelay` no-op params kept), badge.js `BadgeTooltip` body swap (export kept), login.js ×2 `disabled={!multiAuth}` form. Gates pass 2026-07-07: rb Tooltip/OverlayTrigger import grep → 0; `OverlayTrigger` residue = exactly footer.js + hoverable-popover.js (C4's); `standard` clean on all 5 touched files. `pages/dev/tooltip-preflight.js` deleted per §12.0. **Interaction QA (§12.6) + open questions 1/5 evidence: pending sox on-device pass** — risk §12.7-1 (rb Dropdown as trigger child until C8b) untested at runtime. Census corrections vs Table A: OverlayTrigger dies in **3** files here, not 5 — footer.js (×4 sites) + hoverable-popover.js are click/hover **Popover** overlays and ride C4; rb Tooltip = 4 sites / 3 files (action-tooltip.js, badge.js, login.js ×2); ActionTooltip consumers = 8 files / 12 sites (internals swap, consumers untouched). Two tooltip populations exist today (§12.0): ActionTooltip sites snap (no fade, opacity .9, popper collisions off + `position:fixed` hack), badge/login sites fade 0.15s (rb `Overlay` defaults `transition=Fade`; opacity 1, popper collisions ON). First application of revised keystone 5: ~150ms `data-starting-style`/`data-ending-style` motion in tooltip.module.css with the `prefers-reduced-motion` off-switch — supersedes the old "fadeIn = only surviving popup animation" gate; the globals `fadeIn` keyframe + `.fade.tooltip` become PR3 deletions (§12.4/§12.8; its only other consumer `.spin.fade-in` is already dead in JSX). Verified in Base UI 1.6.0 source during spec: `delay`/`closeDelay` live on **Trigger** (default 600/0), and `Tooltip.Provider`'s values cascade (`delay ?? providerDelay ?? 600`) — the `_app.js` Provider carries `delay={0} closeDelay={0}` for OverlayTrigger parity.
- **C4 ✅ done (2026-07-07).** `ui/popover.js` + `popover.module.css`: compound API (`Popover` default = click-toggle drop-in for OverlayTrigger(click, rootClose); `.Root/.Trigger/.Content/.Header/.Body` parts; `popupClasses()`/`popoverStyles` exported so PreviewCard consumers share the chrome). Recipe `text-sm max-w-80 rounded-lg` (13.02px→14px nearest step; 320px exact; 0.5rem = BS $border-radius-lg exact); body/header `py-2 px-4` (painted .5rem/1rem exact); chrome module-side via `--bs-body-bg`/`--bs-body-color`/`--theme-borderColor` (PR3 handoff: mechanical rename); keystone-5 150ms motion + reduced-motion off-switch; bordered diamond arrow (12px, only the two protruding edges draw border — the old globals `.popover .arrow::*` rules were BS4-named and dead, §11.7 lesson). Collision avoidance **left at Base UI defaults** (rb popovers ran popper with preventOverflow ON — unlike tooltips). Consumers: **footer.js** ×4 → `<Popover side='top' trigger={…}>` with the four popover constants collapsed to `PopoverBody` elements (inline `fontWeight/fontSize` styles kept verbatim); **hoverable-popover.js** → PreviewCard `Trigger delay={500} closeDelay={300}` with `onShow` moved to `onPointerEnter`/`onFocus` on the trigger span so lazy queries still prefetch during the delay window (consumers untouched); **upvote.js** §6.9 → controlled `Popover.Root` + `Content anchor={target} side='right'`, no Trigger; §4b `btn-close`/`alert-dismissible` strings died (close chrome = `styles.close`, lightning X per alert precedent; `Popover.Header onClose` renders it). Header recipe `py-2 px-4 text-base font-medium` + module border-bottom. Gates pass 2026-07-07: `OverlayTrigger` grep → 0 (comment-only hit in ui/popover.js), rb Popover/Overlay import grep → 0, upvote btn-close strings → 0, `standard --fix` clean ×4 files; upvote keeps `Dropdown` (C5), footer `nav-link` strings stay (C10). **QA pending sox**: hover-card 500/300 feel, walkthrough popovers anchor right of bolt, footer popovers at viewport bottom edge, dark-mode chrome, arrow notch on all four sides.
- **C5 ✅ done (2026-07-07).** `ui/dropdown.js` + `dropdown.module.css` on Base UI Menu, rb-shaped compound API (`Dropdown/.Toggle/.Menu/.Item/.Divider`; `modal={false}` hardwired; `align` context → Positioner; sideOffset 2). Key design: **dual-mode Item** via `InMenuContext` — in-menu renders `Menu.Item`/`Menu.LinkItem` (`closeOnClick` set — LinkItem defaults false but SN menus persist across client-side nav), standalone renders the same skin as plain a/Link/button (offcanvas list works pre-C10). Item padding rides `--sn-dropdown-item-py/px` vars (module-side, so no §11.0 utility freeze): `.roomy` menu variant (ActionDropdown passes it) replaces the deleted `item.module.css .other :global(.dropdown-item)` consumer rule — **intended delta: ALL ActionDropdown menus (modal, territory/user header) get the .5rem items item-info had, not just `.other` contexts**; offcanvas inline `--bs-dropdown-*` vars renamed to the `--sn-*` pair (its divider-bg var was already dead — globals' `.dropdown-divider` overrode border-top wholesale). Toggle analysis: rb `as='a'` never painted btn classes (Button is *replaced* by `as`, verified in rb source) and `variant='custom'` was a class Bootstrap never generated ⇒ wrapper accepts-and-drops `variant`; module `.toggle` neutralizes UA button chrome (padding 0 — small spacing delta on MeDropdown vs old nav-link padding, QA). Menu chrome recipe `py-2 rounded-md min-w-40 text-base` (exact/nearest per C2.5), module bg `--theme-inputBg` + border + `max-width: 90vw` + keystone-5 motion; `menuClasses()`/`dropdownStyles` exported for the non-Menu deviations. Consumers: action-dropdown.js internals (`styles.dropdown` was an undefined class — dropped); item swaps in bookmark/delete/dont-link-this/mute/share/subscribe/subscribeUser/territory-transfer/carousel/territory-header/upvote (completes the file)/item-info (raw `dropdown-item` Links + ×5 `hr.dropdown-divider` → parts) + **item-job.js ×1 divider the §4b census missed**; nav/common.js MeDropdown/AnonDropdown (Nav/Navbar stay rb until C10); §6.8 split login on inline Menu parts + `dropdownExtra*` skins over `dropdownStyles.menu` base (no −1px collapse: `$btn-border-width: 0`) — **`ui/button.module.css` gained `.twitter`/`.dark` skins** (C1's census missed them because login-button.js was C5-deferred; globals' white-text `!important` baked in as plain white); ToC → controlled Popover + filter per D5 (rb `FormControl` stays until C9a per Table A); mentions → plain listbox per D4 (editor.module.css `suggestionsMenu` opacity hack dropped — dead with rb). Gates pass 2026-07-07: rb Dropdown residue = exactly form.js (D3/C9a) + toolbar/index.js (C8b); raw dropdown-class strings → 0; ButtonGroup → 0; standard clean ×20 files. **QA pending sox**: no scroll-lock, viewport-edge collision, keyboard nav (new: arrows/typeahead), `onPointerDown preventDefault` triggers still open (ActionDropdown/split-login), close-on-select for nav LinkItems, offcanvas item padding, roomy density delta, split-login menu skins, mentions/@~ flow, ToC filter + heading nav.
- **C6 ✅ done (2026-07-07).** `modal.js` render layer → one controlled `Dialog.Root` (D1: `open` constant, unmounts with the stack; `onOpenChange` ignores close when `keepOpen`), all stack/back/replaceModal/persistOnNavigate/onClose ordering logic byte-identical. Anatomy: `Backdrop` (z-backdrop, black .5, entry fade) + `Viewport` (z-modal, the Bootstrap-style scroll container; fullscreen variant carries the old `.modal.fullscreen` inputBg-66% tint) + `Popup` = the dialog panel (`rounded-lg` recipe; module: 500px cap, .5rem/1.75rem-auto margins, inputBg + borderColor, 150ms fade+scale entry per keystone 5 — **exit is snap**: the stack unmounts immediately, matching the $enable-transitions:false era). New `modal.module.css` absorbs every globals `.modal-*` block under local names; §4b `modal-btn/back/close/overflow` strings dead — including a **census-missed site: wallets send `destination-actions.js` scanner close** (now imports the module). Dead-rule finding (§11.7 method): globals' `.modal-close+.modal-body { padding-top: .5rem }` never matched (the close button was never the body's adjacent sibling) — not ported, body keeps `padding: 2rem`. **Ladder revision: `--sn-z-dropdown` 1000 → 1060** — Base UI portals menus to `<body>`, so menus opened from *inside* a modal (modal overflow ActionDropdown, territory-header's inside the sub info modal) must out-stack the modal viewport; Bootstrap dodged this by keeping menus inside the modal DOM. Comment updated in tailwind.css; PR3 handoff unchanged otherwise. Gates pass 2026-07-07: rb Modal import → 0, `modal-*` string grep → 0, module-css `:global(.modal*)` grep → 0, standard clean. **QA pending sox** (§9): zap→QR→back stack (unmount-before-onClose ordering), keepOpen ignores Esc/outside-press, persistOnNavigate, fullscreen carousel + QR scanner, overflow menu stacks above the dialog, long-modal scroll, focus return on close.
- **C7 ✅ done (2026-07-07).** Toast render layer → plain divs `role='status'` per D2; the provider/state machine (dedup `(N) msg` via tagReducer, `mapHidden`, `persistOnNavigate` filter, `animationDelay` resync for unhidden toasts) is byte-identical. New `ToastView` carries autohide as a mount-scoped timer — merged toasts remount under the new toast's id (key change) restarting it, exactly rb's remount semantics; X-close still calls `toast.onClose?.()` first, autohide doesn't (parity). Module absorbs what rb/Bootstrap painted: container gains fixed bottom-end + `--sn-z-toast` + `gap: .5rem` ($toast-spacing; replaces BS's sibling margin-top) + pointer-events none/auto pair; toast gains `box-shadow` ($toast-box-shadow) + per-variant `background-color` (was rb's `bg-*` prop; progressBar's own bg rules still win by specificity); body = `py-3 px-5 wrap-break-word` recipe (globals' `.toast-body` `.75rem/1.25rem !important` exact); radius `rounded-md` ($border-radius .4rem nearest). **Intended delta:** warning-toast text `text-dark`→`text-black` (#212529→#000 — text-dark had no Tailwind token and would have died silently at PR3). Slide/progress keyframes untouched (keystone 5 exemption). Gates pass 2026-07-07: rb Toast imports → 0, standard clean. **QA pending sox**: dedup count + progress-bar resync on merge, warning contrast, autohide 5s, danger persists, X-close onClose chaining, stacking above modal (z 1090), navigation clears non-persist toasts.
- **C8a/b/c ✅ done (2026-07-07 — landed as ONE commit, deviating from the three-way split; each piece is §6.1–6.3-verbatim and separable in review by file).**
  - **C8a (switch.js)**: `Tabs.Root value onValueChange` + `Tabs.List > Tabs.Tab`, no Panels (editor body is the panel); `onMouseDown preventDefault` kept on the List; `disabled={activeTab}` hack dropped (command handler no-ops same-mode). `editor.module.css .modeTab` re-targeted `:global(.nav-link.active)` → `[data-selected]` and, since Tabs.Tab is a `<button>`, absorbed the nav-link paint (navLink/navLinkFocus colors, bold-when-active from globals `.nav-link.active`, UA-chrome reset). The `--tab-*` var rules (focus glow, non-top-level tint) untouched.
  - **C8b (toolbar/index.js)**: `Toolbar.Root` on the `innerToolbar` row (formatting div keeps `toolbarRef` + overflow ResizeObserver untouched); every item → `Toolbar.Button` (roving tabindex = D8 upgrade); dividers → `Toolbar.Separator` (gains role=separator); ToolbarDropdown → `Menu.Root modal={false}` controlled by `dropdownOpen` (kept only to feed ActionTooltip `disable`), trigger composed `Toolbar.Button render={<Menu.Trigger/>}`, `Positioner side='top' align='start' sideOffset={2}` on the ui-dropdown positioner z; popup = ui `menu` base + `dropdownExtra` skins (login-button precedent); items → `Menu.Item` keeping `dropdownExtraItem` skins + `onPointerDown preventDefault` (Lexical selection preservation). **`MenuAlternateDimension` + `useIsClient` guard died.** `transition` props dropped from the 3 tooltip sites (§12.2 obligation). editor.module.css: `.toolbarItem` gained button UA reset; dead rb selectors deleted (`:global(.dropdown-toggle)` ×3 rules, and the `:global(.dropdown) + .divider` nested rule — already unmatched even under rb: buttons always sat between dropdown and divider).
  - **C8c (link/editor.js)**: controlled `Popover.Root open modal={false}` + `Positioner anchor={() => editor.getElementByKey(nodeKey)} side='bottom' align='start' sideOffset={8}` + `Popup initialFocus={false}`; `onOpenChange(false)` → `handleCancel()` (strips empty/default links); Lexical `KEY_ESCAPE_COMMAND` handler kept; edit-mode focus/select effect kept. Killed: `setFloatingElemPosition` + rAF/scroll/resize listeners + focusout pair + the opacity-until-positioned hack (module now has a `.positioner` z rule instead; **`lib/lexical/utils/position.js` deleted — zero consumers remained**). `$updateLink` retains only URL/mode sync; node-gone hiding rides the anchor fn returning null.
  - Gates pass 2026-07-07: rb imports in editor tree = only `editor.js` BootstrapForm (C9a's); standard clean ×3. **QA pending sox** (§9 additions): selection survives menu-driven block format, menu opens above, tooltips suppressed while menu open, Escape → focus back to editor, write/compose parity + upload-block toast, link editor full checklist (§9), mentions still work next to the new popover (BLUR_COMMAND interplay).
- **C9a ✅ done (2026-07-07).** `components/form.js` → 3-line barrel over new `components/form/` (form.js, field.js, input.js, input-group.js, checkbox.js, checkbox-group→checkbox.js, select.js, range.js, variable-input.js, multi-input.js, password-input.js, copy.js, submit-button.js, suggest.js, sn-input.js, date-picker.js, use-field-draft.js, index.js + `controls.module.css`); every original export preserved (verified: all consumer import names resolve against index.js), logic byte-for-byte, presentation swapped. Key builds: **field.js primitives** — `controlClasses()` (module skin + `px-4 py-1.5 text-base rounded-md`, honoring the C9a/C2.5 alignment contract with Button md; sm = button sm exactly), `FormControl` (isInvalid/isValid→module state classes), `FormLabel`/`labelClasses` (92%-bold → `text-sm font-bold mb-2` + module normal-weight `.text-muted` rule), `FormText` (`mt-1 text-sm text-muted`, cn so text-warning overrides), `Feedback` — **renders conditionally via a `show` prop instead of rb's `.is-invalid ~ .invalid-feedback` CSS sibling gate**, `FormGroup` (`mb-4` ≡ globals .form-group). **input-group.js** — flex module + corner-joining by APPENDING positional radius utilities to children via cloneElement (risk 7: module radius loses to layered-important recipe utilities; appended longhands beat the shorthand in TW's canonical order); `size` forwarded only to component children (it's the char-width attr on DOM inputs); feedback moved OUTSIDE the group (conditional now, sibling hack dead). **controls.module.css** — .form-control/.form-check-*/.form-select/.input-group-text painted values under local names (check glyphs baked black per SN's `$form-check-input-checked-color`; select block kept verbatim as a deliberate SN one-off incl. the `calc(1.5em+.5rem+2px)` height + `.7875rem` font). **Drafts** → `use-field-draft.js` (same `${prefix}-${name}` keys). **Suggest (D3)** — logic 100%, render → caret-anchored plain listbox on the ui-dropdown chrome; `form.module.css .suggestionsMenu` opacity hack deleted. **Radios** = §6.10(a) native (type='radio' through the same Checkbox). Legacy-shaped movers for C9b: Range (native range keeps the interim `form-range` class — Bootstrap CSS serves it until the C9b Slider), CheckboxGroup (feedback wrapper), MultiInput. Consumer sweep: 15 InputGroup files + 5 deferred C1 files (user-header, item-act, settings, rewards, wallets actions — rb Button→ui; **actions.js:55 → buttonClasses Link, and the Button `href`/`as` shim is DELETED** per the C1 note); BootstrapForm users swapped (avatar → plain interim `form-range`, multi-select/new-header/territory-form/wallets-search/editor.js → field primitives; editor.js + multi-select + local-draft import **leaf modules** (`form/field`, `form/form`) to dodge the barrel→sn-input→editor cycle); ToC FormControl swapped. §4b strings dead + census-missed finds: `form-label` divs in ots.js ×3 / settings ×3 / logins ×1 (→ `labelClasses()`), `invalid-feedback` in wallets destination-input (→ Feedback). Gates 2026-07-07: rb residue = exactly Nav/Navbar ×13 + Offcanvas ×2 (C10) + Accordion (C11); form-control/form-label/invalid-feedback grep → 0; standard clean ×30. **Intended deltas:** input px .84rem→1rem (contract), label 13.7px→14px, feedback/hint .875em→text-sm, `.input-group-text` loses the <767px anti-zoom font bump (PR3 rewrites that media query anyway). **QA pending sox:** drafts persist/restore/clear, invalid-after-submit-only, @/~ autocomplete keyboard + click-with-blur-grace, InputGroup corner joins (copy input, password eye, sats addons, user-header nym edit, MultiInput sequence), checkbox/radio paint + territory postTypes, select paint (nav sub select esp.), datepicker light+dark, search page dual autocomplete, /email MultiInput, wallet forms (protocol fields, max-fee, lightning-address), fee-button inside post form.
- **C9b ✅ done (2026-07-07).** **Range → Slider + NumberField** (§6.6 verbatim): Formik single source of truth, both widgets controlled from `field.value`, user-event-only writes; ∞ sentinel transfers (`v <= sliderMin → setValue(null)`, chip rendered when null); NumberField `format={{ useGrouping: false }}`, native clamp-on-blur replaces the hand-rolled handler, hide-spinners hack dead (text input); tick-label percent math + outer grid unchanged; slider skin in controls.module.css (track = `--theme-inputDisabledBg` ≈ BS secondary-bg, thumb = `--bs-primary` so territory retint flows, 50% primary-mix Indicator — a nearest-native addition BS's form-range didn't have, gold focus ring). **Bonus**: avatar.js's zoom range (missed by Table B) → inline Slider on the same skin (its interim `form-range` class would have gone naked at PR3). **MultiInput → OTP Field**: public API frozen (sole consumer pages/email.js), `validationType` numeric/alphanumeric by `inputType`, `normalizeValue` carries `upperCase`, `autoSubmit` OFF (§9 parity), inputs = `controlClasses() text-center w-11` (44px exact), first-input autoFocus; ~80 lines of focus bookkeeping died; `charLength`/`showSequence` accepted-and-ignored (no consumer). **CheckboxGroup: NEW DEVIATION D11 — stays the Formik feedback wrapper, NOT Base UI CheckboxGroup.** Rationale (census 2026-07-07): the billing "CheckboxGroup" contains **radios bound to a different field** (`billingType` vs group `name='billing'`) which Base UI's group can't express, and postTypes array membership is Formik-native via per-checkbox `value` + shared name — a Base UI group would double-manage state (echo-loop family, risk 3) for zero user-visible gain. Table B's row is amended by this entry; the §9 QA target (postTypes toggles, error-after-submit-only) is unchanged. Gates 2026-07-07: `form-range`/`hide-spinners` grep → 0 (comment-only), standard clean. **QA pending sox**: §9 Range row (∞ both directions, typed number moves slider, blur clamps, no grouping, territory+settings submit identical values), /email code entry (type-advance/backspace/paste/no-autosubmit), avatar zoom, slider keyboard arrows (new, native).
- **C10 ✅ done (2026-07-07).** **ui/nav.js** — plain markup + active-key context (`Nav`/`Nav.Item`/`Nav.Link`/`Navbar`/`Navbar.Brand`, `navLinkClasses()` for raw sites); module carries the globals nav paint (navLink/navLinkFocus colors, bold-active, navItem 500, navbar geometry + the stacked-header padding-collapse rules under `:global(header)`). **New consumer-override mechanism (ownership rule made concrete):** Nav.Link emits `data-nav-link`/`data-active` attributes — consumer modules key off those instead of racing our module classes: `styles/nav.module.css` re-targeted (`.nav :global .nav-link/.active` → `[data-nav-link]`/`[data-active]`, the settings/satistics/user-header tab underline), `header.module.css .navLink` bumped to `.navLink[data-nav-link]` (deterministic (0,1,1) > our (0,1,0)); its dead `.navLink.active svg` rule (localized `.active` no JSX ever applied) deleted, not ported. **ui/drawer.js** — Offcanvas drop-in on Base UI Drawer, end + bottom, controlled; **deviation D12: native swipe-to-dismiss ON** (Base UI has no swipe-off switch — their docs route gesture-free panels to positioned Dialog — and swipe-dismiss is a D8/D9-family mobile upgrade; fallback if QA dislikes: positioned Dialog). Motion = 150ms family (Drawer's 450ms default shortened per keystone 5, swipe-movement vars wired); z = ladder rungs (the old module z **10000** dropped); chrome module-side (`--bs-body-bg`, borderColor edges, lightning-X close via `Drawer.Close`); header/title/body recipes (`p-4`, `text-lg`). Consumers: mobile nav offcanvas (module keeps only `max-width: 250px` — its slide keyframe/z/right hacks deleted) + wallets bottom sheet (`.sheet` drops `height:auto !important` — no BS to fight; its dead `:global(.offcanvas-body)` rule deleted — the overflow-x:visible it asked for is a CSS impossibility next to overflow-y:auto). Import swaps ×13 nav files (JSX untouched — API mirrored); **§6.11 Indicator literal ternary** (`bg-danger`/`bg-secondary` now scanner-visible — PR3 §8b's runtime-class burden shrinks to the `text-*` family); §4b `nav-link` sweep ×~30 sites incl. census-missed ones: use-comments-navigator, wallets logger + passphrase ×2, referrals + footer `nav-item` smalls (→ `font-medium`, the .nav-item paint). Gates 2026-07-07: rb residue = **accordian-item.js ONLY** (C11); nav-link/nav-item raw strings → 0; standard clean. **QA pending sox:** all four header bars + sticky bar at both breakpoints, brand/back, price/wallet summary spacing (module `.navLink` padding is now uniform .5rem — old navbar-x .5/plain-nav 1rem split gone: check settings/satistics/user-header tab gaps), sort underlines, comments sort bar, mobile drawer (end) incl. swipe-dismiss + avatar/backdrop, wallets bottom sheet + swipe-down, MeDropdown/AnonDropdown toggles (navLinkClasses now), footer link colors/hovers.
- **C11 ✅ done (2026-07-07) — rb-zero.** `ui/collapsible.js` (+150ms height-motion module via `--collapsible-panel-height`); **accordian-item.js** internals: `AccordianItem` → controlled Collapsible (arrow flip on `open`, `show` followed both directions like the old double effect, **`<div key={open}>` body-remount parity kept**, trigger = `role='button' tabIndex={0}` div — keyboard toggle is a D8-family gain); `AccordianCard` → Collapsible + new `accordian-item.module.css` painting `.accordion-item/-button/-body` (SN's `$accordion-padding-y: 0`; caret = the pinned #808080 chevron rotating on `[data-panel-open]`). **long-pressable.js** functional rewrite (same props/pointer semantics; PureComponent + PropTypes died). **`bs-utility-check.js --components`** landed (§4b gate): strict component-class blocklist (btn/form-*/dropdown-*/nav-*/modal-*/alert/badge/input-group/offcanvas/accordion/toast/…) flagged in class contexts; playground skipped wholesale (deliberate comparison page, dies before the PR). The gate immediately paid for itself: **`btn-sm` on mobile-footer PostItem** (→ `size='sm'` prop threaded through buttonClasses), **raw `form-group`** in job-form (→ `mb-4`), **toast body `py-3 px-5`** (my C7 recipe collided with BS-era spacing names → module `.body`), and **notifications retry `border`** (bare border is a BS-only shape to the checker; → `border-1`, the same painted value). Gates 2026-07-07: `grep react-bootstrap components pages wallets lib` → **0 imports**; `--components` → **0**; standard clean. react-bootstrap stays in package.json until PR3 Phase D (plan §9). **QA pending sox**: all 16 AccordianItem consumers (adv-post accordions, territory form, past bounties, related, lightning/nostr auth accordions…), territory-header AccordianCard chrome, upvote long-press (functional rewrite) incl. drag-cancel + 500ms timing.
- **C12 ✅ done (2026-07-07).** **Capability switch** → `Switch.Root`/`Switch.Thumb` inside the existing label (buttons are labelable — text clicks still toggle); module re-targeted `:checked +` → `[data-checked]`, `::after` thumb became a real `.toggleThumb`, track carries button resets; manual `role='switch'` markup died. **bolt11 pills (§6.7)**: `ExpandableDetailPill` → controlled Collapsible (Trigger = the pill button, free `aria-expanded`; `+/-` indicator + title stay JSX-driven off the state); **more/less chip row** → controlled `Collapsible.Root` + **`Panel className='contents'`** so overflow chips join the flex-wrap row with zero layout shift — the toggling Chip stays an external control (tiny deviation from the sketch: `Chip` doesn't forward refs, so it isn't the literal Trigger; behavior identical, panel semantics carry the expanded state). **max-fee toggle** → controlled Collapsible, Trigger = the chip button, icon swap reads state. **Tips → Toggle Group (§6.12 verbatim)**: pressed state DERIVED from the `amount` field via `useField` (never stored), `onValueChange` ignores empty arrays (clicking the pressed chip = visual no-op), `ToggleGroup className='contents'` keeps the flex-wrap row, chips = `Toggle` styled by `buttonClasses({ size: 'sm' })`; **`.btn[data-pressed]` skin added to button.module.css** (active-formula reuse — D9's deliberate new affordance); localStorage custom-tips logic untouched. Panels all ride ui/collapsible's 150ms height motion. Gates 2026-07-07: standard clean; manual aria-expanded/role=switch grep → comment-only. **QA pending sox** (§9 additions): capability toggle + label click, pill expand/collapse in payment modals, more/less with >6 chips (no row reflow), max-fee show/hide + field value persistence, Tips full checklist (chip sets amount, typed custom clears pressed, pressed chip click keeps value, custom tips sorted).
- **C13 (optional MultiSelect → Combobox) ⬜ not taken** — react-select stays per D10 (droppable, doesn't block PR3).

**PR2 exit status (2026-07-07): C0–C12 all landed — zero react-bootstrap imports, §4b component classes zero, all mechanical gates green.** Outstanding before merge: sox's interaction/visual QA passes accumulated per-commit above (§9 checklist), the visual-diff harness run, and deleting `pages/dev/playground.js`.
