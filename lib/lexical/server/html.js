 ...
 * @param {string|number} [options.entityId] - optional entity identifier for error logging
...
export function lexicalHTMLGenerator (lexicalState, options = {}, editorOptions = {}) {
  ...
  return withDOM(window => {
    ...
    try {
      ...
      console.error(`error parsing Lexical editor state${idLabel}: `, error)
    ...
    editor.update(() => {
      try {
        ...
        console.error(`error generating HTML in SSR from Lexical State${idLabel}: `, error)
        html = null
      }
    })
