...
export function lexicalStateLoader ({ me, userLoader } = {}) {
  ...
  return new DataLoader(
    async (keys) => {
      ...
      return Promise.all(
        keys.map(({ text, entityId, context }) => {
          ...
          if (netInvestment !== undefined) {
            ...
            outlawed = me?.id !== userId && threshold != null && (netInvestment ?? 0) < threshold
          }
          const showImagesAndVideos = userFilters.showImagesAndVideos
          const imgproxyOnly = userFilters.imgproxyOnly
          return prepareLexicalState({ text, entityId, context: { ...rest, outlawed, showImagesAndVideos, imgproxyOnly } })
            .catch(error => {
