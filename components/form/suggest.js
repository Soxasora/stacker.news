...
// deviation D3: the selection/keyboard logic is 100% preserved; only the
// render swapped — a caret-anchored plain listbox instead of rb Dropdown
// (kills the opacity !important workaround)
  ...
  return (
    <>
      ...
      {suggestions.array.length > 0 && (
        // dropdownStyle carries the caret-anchored absolute position (from
        // useEntityAutocomplete); without it the wrapper anchors in-flow
        // under the input, like the old toggle-less rb Dropdown did
        <div className='relative' style={{ ...dropdownStyle, zIndex: 'var(--sn-z-dropdown)' }}>
          <div role='listbox' className={cn(menuClasses(), 'absolute top-full start-0 mt-0.5')}>
            {suggestions.array.map((v, i) =>
              <div
                role='option'
                aria-selected={suggestions.index === i}
                key={v.name}
                className={cn(dropdownStyles.item, suggestions.index === i && dropdownStyles.active)}
                onClick={() => {
                  onSelect(v.name)
                  resetSuggestions()
                }}
