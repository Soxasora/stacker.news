...
function appendSubWithPlus (existingSub, newSub) {
  if (existingSub.includes(newSub)) return existingSub
    ...
    <MultiSelect
      onChange={onChange || ((_, e) => {
        ...
        if (router.query.sub) {
          ...
          // if we are going to a sub, replace the current sub with the new one
          asPath = router.asPath.replace(`/~${router.query.sub}`, sub ? `/~${appendSubWithPlus(router.query.sub, sub)}` : subReplace)
          // if we're going to home, just go there directly
          if (asPath === '') {
            router.push('/')
            return
        ...
        const query = {
          ...router.query,
          sub: router.query.sub ? appendSubWithPlus(router.query.sub, sub) : sub
        }
        delete query.nodata
        router.push({
          pathname: router.pathname,
